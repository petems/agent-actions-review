...
function statusIcon(status, conclusion) {
  ...
  if (status === "queued" || status === "waiting" || status === "pending") return `${colors.blue}◌${colors.reset}`;
...
function formatRunDetail(run, jobs) {
  ...
  lines.push(`${colors.dim}Branch: ${run.branch}  SHA: ${run.sha}  Event: ${run.event}${colors.reset}`);
  ...
  for (const job of jobs) {
    ...
    lines.push(`  ${jobIcon} ${jobColor}${job.name}${colors.reset} ${colors.dim}(${job.conclusion || job.status})${colors.reset}`);
...
function formatJobDetail(job, logExcerpt) {
  ...
  if (logExcerpt && logExcerpt.errors && logExcerpt.errors.length > 0) {
    ...
    if (logExcerpt.truncated) {
      lines.push(`${colors.yellow}(log truncated, ${logExcerpt.totalLines} total lines)${colors.reset}`);
    }
...
function formatWatchStatus(pollCount, summary) {
  ...
  if (summary.pending > 0) parts.push(`${colors.yellow}${summary.pending} in progress${colors.reset}`);
