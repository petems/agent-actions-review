  ...
  for (let i = 0; i < sortedLines.length; i++) {
    ...
    if (currentRange.length === 0 || lineNum === sortedLines[i - 1] + 1) {
      currentRange.push({
        ...
        text: lines[lineNum],
        isError: errorLineNumbers.has(lineNum),
      });
    } else {
      errors.push(currentRange);
      currentRange = [
        { lineNumber: lineNum + 1, text: lines[lineNum], isError: errorLineNumbers.has(lineNum) },
      ];
    }
  }
  ...
  for (const pattern of INFRA_PATTERNS) {
    if (pattern.test(combined)) {
      return {
        category: "INFRA",
        confidence: "high",
        reason: `Infrastructure issue: ${pattern.source}`,
      };
    }
  }
...
async function areAllChecksPassing(owner, repo, branch, token, proxyFetch) {
  ...
  for (const run of runs) {
    ...
    if (
      !latestByWorkflow.has(key) ||
      new Date(run.createdAt) > new Date(latestByWorkflow.get(key).createdAt)
    ) {
